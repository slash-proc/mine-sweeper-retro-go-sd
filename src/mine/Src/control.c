    ...
    switch (nowMode)
    {
        ...
        case MODE_GAMEOVER:
            ...
            if (flashFlag == 1) {
                ...
                {
                    DISP_flashGameOver(flashOnOff, (isWin && lastGameTime < oldSaveDataBestTime) ? TRUE : FALSE , "Easy");
                } else if (gameLevel == GAME_LVL_2) {
                    DISP_flashGameOver(flashOnOff, (isWin && lastGameTime < oldSaveDataBestTime) ? TRUE : FALSE , "Normal");
                } else if (gameLevel == GAME_LVL_3) {
                    DISP_flashGameOver(flashOnOff, (isWin && lastGameTime < oldSaveDataBestTime) ? TRUE : FALSE , "Hard");
                }
            }
            break;
        case MODE_INFO:
    ...
    case MODE_BEFROE_GAMEOVER:
        ...
        case KEY_EVENT_DOWN:
            ...
            if (gameLevel == GAME_LVL_1)
            {
                ...
                DISP_drawGameOver(isWin, "Easy", lastGameTime, oldSaveDataBestTime);
            } else if (gameLevel == GAME_LVL_2) {
                oldSaveDataBestTime = gSetting.hiScoreLvl2;
                DISP_drawGameOver(isWin, "Normal", lastGameTime, oldSaveDataBestTime);
            } else if (gameLevel == GAME_LVL_3) {
                oldSaveDataBestTime = gSetting.hiScoreLvl3;
                DISP_drawGameOver(isWin, "Hard", lastGameTime, oldSaveDataBestTime);
            }
